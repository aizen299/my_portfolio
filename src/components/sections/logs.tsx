...
/**
 * 07 — Logs. Homepage teaser for the blog: the latest three posts as
 * compact rows plus a prominent "read all logs" link to /blog. The full
 * index and post routes live under src/app/blog/.
 */
      ...
      <ul className="mt-16 max-w-3xl border-t">
        {posts.map((post) => (
          <li key={post.slug}>
            <Link
              href={`/blog/${post.slug}`}
              className="group flex flex-col gap-2 border-b py-8 sm:flex-row sm:items-baseline sm:justify-between sm:gap-8"
            >
